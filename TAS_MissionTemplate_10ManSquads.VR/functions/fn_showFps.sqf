...
private _position = missionNamespace getVariable ["PROF_numberFpsDisplaysActive",0];
missionNamespace setVariable ["PROF_numberFpsDisplaysActive",_position + 1,true];

if (_position > 5) then {
    [format ["PROF_showFps: selected position (%1) is > 5, map marker may not be visible due to excessive distance from map!",_position]] call PROF_fnc_error;
};
...
[format ["PROF_showFps: Executing on %1 with timeout of %2 and position of %3.",_sourcestr,_timeout,_position]] call PROF_fnc_error;
