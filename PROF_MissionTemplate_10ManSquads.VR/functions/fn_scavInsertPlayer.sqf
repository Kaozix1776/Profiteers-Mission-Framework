...
if !(isServer) then {
	[format ["fn_scavInsertPlayer is being executed on %1 instead of on the server, errors will likely occur!!",name player],false] call PROF_fnc_error;
};

private _availableExtracts = missionNamespace getVariable ["PROF_scavExtracts",[0,0,0]];
private _spawnHouse = [] call PROF_fnc_scavFindSuitableBuilding;
...
if (_houseTpPosAGL isEqualTo [0,0,0]) then { //fallback, spawn near extract
	...
	["fn_scavInsertPlayer houseTpPosAGL is invalid, performing backup spawn"] call PROF_fnc_error;
} else {
