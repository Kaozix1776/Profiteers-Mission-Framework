...
if (_debug) then {[format ["Units: %1",_units]] call PROF_fnc_error};
{
	if (_doScale) then {
		...
		if (_debug) then {[format ["Unit: %1, Distance: %2, Factor: %3, Intensity: %4",_x,_distance,_factor,_specificIntensity]] call PROF_fnc_error};
	} else {
		_specificIntensity = _intensity;
	};
	_specificIntensity = (floor _intensity) + 1;
	if (_debug) then {[format ["Unit: %1, Intensity: %2",_x,_specificIntensity]] call PROF_fnc_error};
