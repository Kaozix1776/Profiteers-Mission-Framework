...
if (!isNil "AceHealObject") then { //check if the ace heal object actually exists so we dont get errors
	...
	if (PROF_respawnArsenalGear && !(PROF_vassEnabled)) then {
		_actionID = [
			AceHealObject,											// Object the action is attached to
	...
	if (PROF_playerStorage) then {
		...
		PROF_holdActionIDs pushBack [AceHealObject,_actionID,"open_storage"];

		if !(PROF_storageFullSave) then { //Don't bother to save if we're already autosaving
			_actionID = [
				AceHealObject,											// Object the action is attached to
		...
		if (!isNull _object) then {
			...
			_actionID = [
				...
				{},													// Code executed on every progress tick
				{
					...
					private _cost = [PROF_vassShops select 0] call PROF_fnc_vassGetLoadoutCost;
					PROF_rebuyCost = _cost * PROF_vassRebuyFactor;
