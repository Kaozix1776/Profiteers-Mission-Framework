...
if !(isServer) then { //only execute stuff with player on player machines
	...
	private _retrievedMoneyForEnd = profileNamespace getVariable [PROF_vassShopSystemVariable,0];
	hint "MISSION SAVE BEGINNING, EXPECT SMALL FREEZE WHILE VARIABLES ARE SAVED. DO NOT DISCONNECT UNTIL MISSION ENDS!";
	sleep 3;
	profileNamespace setVariable [PROF_vassShopSystemLoadoutVariable,getUnitLoadout player]; //save loadout for next mission, money already saves automatically
