...
private _onConfirm =
{
	...
	if (count _side > 1) exitWith {["Error when executing Zeus Spawn Scav Group module: select only 1 side!",false] call PROF_fnc_error};
	...
	[format ["PROF_fnc_zeusSpawnScavGroup executing with side %1, group %2, unitClass %3, number of units %4, and position %5!",_side,_group,_unitClass,_numberUnits,_pos]] call PROF_fnc_error;
	...
	for "_i" from 1 to _numberUnits do {
		...
		[_unit,_magazines,_radio,_grenades,_changeSkill] call PROF_fnc_scavLoadout;
	};
