...
//////////////////////////////////
	...
	/* Persistant Player Storage
		Allows each player to use a persistant storage box that is unique to them.
			...
			PROF_playerStorage controls whether the system is enabled/disabled.
			PROF_playerStorageVar is the unique identifier used to save storage contents unique to your campaign.
				Change this to a (short) name of your campaign. Try 5 characters or less.
			PROF_storageClassname is the classname of the box used. This controls how much inventory space is available.
			...
			PROF_storageLocation is the location of the debug corner to spawn the boxes at.
			PROF_storageFullSave controls whether a full save of profileNamespace is done after every time the box is opened.
		...
		*/
		PROF_playerStorage 		= true; 	//default false
		PROF_playerStorageVar 	= "Test"; 	//default "Test", CHANGE THIS (see above)
		PROF_storageClassname 	= "Box_IND_Support_F"; //default "Box_IND_Support_F"
		PROF_storageLocation 	= [0,0,0]; 	//default [0,0,0] (map origin)
		PROF_storageFullSave 	= true; 	//default true
		PROF_storageOpenAnywhere = false; 	//default false
	...
	//////////////////////////////////
		...
		/* Virtual Arsenal Shop System (VASS)
			...
			*/
			...
			PROF_vassShops 				= ["arsenal_1","arsenal_2"]; //variable names of shop objects. These should ideally have the same shop contents, as rebuy cost is calculated from the first one in the list. note: if doing limited quantities and multiple shops, quantities will probably not be shared between shops
			PROF_vassSigns 				= ["AceHealObject"]; 		//variable names of rebuy sign objects
			PROF_vassRebuyFactor			= 0.5; 						// Percentage amount (in decimal form, so 1 = 100%) of total loadout value that a rebuy should cost. Default: 0.5 (50%)
