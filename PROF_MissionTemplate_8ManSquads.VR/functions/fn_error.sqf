...
private _prefix = "PROF-MISSION-TEMPLATE: ";

if (typename _msg != "STRING") then {
    ["ERROR: PROF_fnc_error called with a non-string message variable! Attempting to stringify parameter...",true] call PROF_fnc_error;
    _msg = str _msg;
};
...
if (_adminOnly) then {
    if (isServer || (serverCommandAvailable "#logout") || (!isNull (gePROFsignedCuratorlogic player))) then {
        systemChat _msg;
    };
} else {
    systemChat _msg;
