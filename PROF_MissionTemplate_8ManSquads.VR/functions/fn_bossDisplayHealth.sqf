...
//[] spawn PROF_fnc_displayHealth;
private _debug = false;

if !(PROF_bossEnabled) exitWith {
	diag_log "PROF MISSION TEMPLATE: attempted to display boss health system without the boss system being enabled!";
	if (_debug) then {
...
waitUntil {sleep PROF_bossInterval; !(isNil "PROF_bossComponents")};
...
while {true} do { //TODO not true
	private _imag  = format ["<img size='8' image='%1' align='center'/>",PROF_bossImagePath];
	private _output = "<br/><t color='#cc6600' size='3' align='center'>MECH HEALTH DISPLAY</t><br/><br/>";
	
	{
		private _varName = format ["PROF_%1",_x];
	...
	} forEach PROF_bossComponents;

	hint parseText (_output + "<br/><br/>" + _imag + "<br/><br/>");

	sleep PROF_bossInterval;
