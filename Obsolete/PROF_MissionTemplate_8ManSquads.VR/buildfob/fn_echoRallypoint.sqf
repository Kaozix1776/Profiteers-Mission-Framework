...
if (PROF_rallyEchoUsed == false) then { "rallypointEchoMarker" setMarkerAlpha 1; };  //first time rally is created, set its marker to visible
if (PROF_rallyEchoUsed == true) then {
	{deleteVehicle _x} forEach PROF_rallypointEcho;
	//PROF_rallypointEchoRespawn call BIS_fnc_removeRespawnPosition;
	private _path = [PROF_respawnLocations, "Echo Rallypoint"] call BIS_fnc_findNestedElement;
...
//PROF_rallypointEchoRespawn = [side player, getPos player, "Echo Rallypoint"] call BIS_fnc_addRespawnPosition; //not private so we can delete later
PROF_respawnLocations pushBack [_rallypointPosATL,"Echo Rallypoint"];
...
if (PROF_useSmallRally == false) then {
	PROF_rallypointEcho = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointComposition.sqf"))] call BIS_fnc_ObjectsMapper; //not private so we can delete later
} else {
	PROF_rallypointEcho = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointSmallComposition.sqf"))] call BIS_fnc_ObjectsMapper;
...
PROF_rallyEchoUsed = true; //tell game that the squad's rally is used and so it must be deleted before being respawned
publicVariable "PROF_rallyEchoUsed"; //might be unneccessary since only 1 person can be SL so don't need public, just exist on SL's machine
										//might also need PROF_rallypointEchoRespawn and PROF_rallypointEcho to be public for functionality in case SL disconnects and is replaced
...
if (PROF_rallypointOverrun) then {
	[_friendlySide,_enemySides,_radius,_nearEnemies,_nearFriendlies,_nearEnemiesNumber,_nearFriendliesNumber,_rallypointPosATL] remoteExec ["PROF_fnc_EchoRallyOverrun",2]; //run overrun on server to avoid issues with FPS or client disconnect
};
