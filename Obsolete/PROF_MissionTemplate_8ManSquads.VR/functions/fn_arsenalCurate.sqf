...
if (_debugCurate) then { systemChat "a"; };
{
	...
	if (!isNull _arsenal) then {
		...
		//PROF Flashlights
		if (isClass(configFile >> "CfgPatches" >> "PROF_BrightLite")) then { 
			if (_debugCurate) then { systemChat "d"; };
			[_arsenal, ["PROF_acc_brightlite_sniper","PROF_acc_brightlite_high","PROF_acc_brightlite_low","PROF_acc_brightlite_static","PROF_acc_nightlite_sniper","PROF_acc_nightlite_high","PROF_acc_nightlite_low","PROF_acc_nightlite_static"]] call ace_arsenal_fnc_removeVirtualItems;
		};

		//PROF Doomsday
		if (isClass(configFile >> "CfgPatches" >> "PROF_Revolver")) then { 
			if (_debugCurate) then { systemChat "e"; };
			[_arsenal, ["PROF_6Rnd_doomsday_Buck"]] call ace_arsenal_fnc_removeVirtualItems; //PROF_6Rnd_FRAG too?
		};
		...
		if (isClass(configFile >> "CfgPatches" >> "LAGO_Biken")) then { //just check for biken main mod b/c lazy and not checking for each of them
			...
			[_arsenal, ["LAGO_Biken","LAGO_KU3K","LAGO_KU5K","LAGO_KU98K_NSK","LAGO_KUoooK","LAGO_SCannon","LAGO_CP"]] call ace_arsenal_fnc_removeVirtualItems; //PROF_6Rnd_FRAG too?
		};
		...
		if (isClass(configFile >> "CfgPatches" >> "GX_M82A2_Weapon")) then { //just check for biken main mod b/c lazy and not checking for each of them
			if (_debugCurate) then { systemChat "f"; };
			[_arsenal, ["GX_M82A2_10Rnd_HE_Mag","GX_M82A2_10Rnd_HEDP_Mag"]] call ace_arsenal_fnc_removeVirtualItems; //PROF_6Rnd_FRAG too?
		};
