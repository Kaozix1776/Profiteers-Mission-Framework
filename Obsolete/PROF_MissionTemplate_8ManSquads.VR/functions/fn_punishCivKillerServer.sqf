/*
	...
	Examples:
	[_civ] remoteExec ["PROF_fnc_punishCivKiller",2];
	[_civ] spawn PROF_fnc_punishCivKiller;
*/
private _debug = false;
params ["_civ"];
if (_debug) then { systemChat format ["Starting fn_punishCivKillerServer with %1",_civ]; };

//if !(PROF_punishCivKills) exitWith { if (_debug) then { systemChat format ["Exitting fn_punishCivKillerServer as system is disabled!"]; }; };

if (_civ getVariable ["PROF_isProtectedUnit",false]) exitWith { if (_debug) then { systemChat format ["Exitting fn_punishCivKillerServer as unit %1 has already been protected!",_civ]; }; };

_civ setVariable ["PROF_isProtectedUnit",true,true]; //dunno if it should be public but may as well
_civ addEventHandler ["Killed", {
	...
	if (isPlayer _killer) then {
		private _kills = _killer getVariable ["PROF_civsKilledByUnit",0];
		_kills = _kills + 1;
		_killer setVariable ["PROF_civsKilledByUnit",_kills,true];
		_msg = format ["You have killed a civilian! Current civilian kills: %1. Civilian kills allowed before punishment: %2",_kills,PROF_punishCivKillsThreshold];
		_msg remoteExec ["systemChat",_killer];
		if (_kills > PROF_punishCivKillsThreshold) then {
