...
if (PROF_rallyReconUsed == false) then { "rallypointReconMarker" setMarkerAlpha 1; };  //first time rally is created, set its marker to visible
if (PROF_rallyReconUsed == true) then {
	{deleteVehicle _x} forEach PROF_rallypointRecon;
	//PROF_rallypointReconRespawn call BIS_fnc_removeRespawnPosition;
	private _path = [PROF_respawnLocations, "Recon Rallypoint"] call BIS_fnc_findNestedElement;
...
//PROF_rallypointReconRespawn = [side player, getPos player, "Recon Rallypoint"] call BIS_fnc_addRespawnPosition; //not private so we can delete later
PROF_respawnLocations pushBack [_rallypointPosATL,"Recon Rallypoint"];
...
if (PROF_useSmallRally == false) then {
	PROF_rallypointRecon = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointComposition.sqf"))] call BIS_fnc_ObjectsMapper; //not private so we can delete later
} else {
	PROF_rallypointRecon = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointSmallComposition.sqf"))] call BIS_fnc_ObjectsMapper;
...
PROF_rallyReconUsed = true; //tell game that the squad's rally is used and so it must be deleted before being respawned
publicVariable "PROF_rallyReconUsed"; //might be unneccessary since only 1 person can be SL so don't need public, just exist on SL's machine
										//might also need PROF_rallypointReconRespawn and PROF_rallypointRecon to be public for functionality in case SL disconnects and is replaced
...
if (PROF_rallypointOverrun) then {
	[_friendlySide,_enemySides,_radius,_nearEnemies,_nearFriendlies,_nearEnemiesNumber,_nearFriendliesNumber,_rallypointPosATL] remoteExec ["PROF_fnc_ReconRallyOverrun",2]; //run overrun on server to avoid issues with FPS or client disconnect
};
